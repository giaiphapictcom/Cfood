(function ($) {
    ...
    var typo = {
        ...
        go: function () {
            ...
            this.goAddToCart();
            this.updateProductQuantityToCart();
            this.goBox();
        ...
        sidebarMapSorting: function (n) {
            $(".browse-tags a").click(function (n) {
                if (!$(this).parent().hasClass("active")) {
                    Shopify.queryParams.sort_b = $(this).attr("href");
                    typo.sidebarAjaxClick();
                    var cart = $(this).text();
        ...
        goWishlist: function () {
            $(".grid-item a.wishlist").click(function (n) {
                n.preventDefault();
                var r = $(this).parent();
                ...
                $.ajax({
                    type: "POST",
                    url: "/MemberWishList/addToWishList",
                    data: "id=" + productId,
                    beforeSend: function () {
                        typo.showLoading()
                    },
                    success: function (n) {
                        ...
                        if (n.code == "ok") {
                            ...
                            $(".ajax-success-cbox").find(".show-wishlist").html(n.message);
                    ...
                    error: function (n, r) {
                        typo.hideLoading();
                        $(".loading").hide();
                        $(".ajax-error-message").text(n.message);
            ...
            $(".product button.wishlist").click(function (n) {
                ...
                $.ajax({
                    ...
                    error: function (n, r) {
                        typo.hideLoading();
                        $(".loading").hide();
                        $(".ajax-error-message").text(n.messages);
                        typo.showBox(".ajax-error-cbox")
                    }
                })
            })
        ...
        goRemoveWishlist: function () {
            $(".btn-remove-wishlist").click(function (n) {
                ...
                $.ajax({
                    type: "POST",
                    url: "/MemberWishList/RemoveItemFromWishList",
                    data: "id=" + i,
                    beforeSend: function () {
                        typo.showLoading()
                    },
        ...
        updateProductQuantityToCart: function () {
            if ($("#cartform").length > 0) {
                $("input[id^='updates_']").change(function () {
                    var productId = parseInt($(this).attr('id').replace('updates_', ''));
                    var quantity = parseInt($(this).val());
                    if (quantity == 0) {
                        $(".ajax-error-message").text("Số lượng sản phẩm phải là giá trị số.");
                        typo.showBox(".ajax-error-cbox");
                    }
                    else {
                        typo.doAjaxUpdateProductQuantityToCart(productId, quantity);
                    }
                    return false;
                });
            }
        ...
        doAjaxUpdateProductQuantityToCart: function (id, quantity) {
            $.ajax({
                type: "post",
                url: "/cart/updatecart",
                ...
                success: function (n) {
                    ...
                    if (n.code == 1) {
                        ...
                        $(".ajax-success-cbox").find(".mesage-cart").text(n.message);
                        window.reload();
        ...
        goQuickView: function () {
            jQuery(".quickview-button a").click(function () {
                ...
                Shopify.getProduct(n, function (n) {
                    ...
                    console.log(n.id);
                    $("form[name='quickview-form']").attr('id', 'product-actions-' + n.id);
                    $("form[name='quickview-form'] input[name=id]").val(n.id);
                    ...
                    if (i.find(".product-inventory span").length > 0) {
                        ...
                        var o = n.inventory_quantity;
                        if (o > 0) {
                            i.find(".product-inventory span").text(o + " trong kho")
                        } else {
                            i.find(".product-inventory span").text("Hết hàng")
                        }
                    }
                    ...
                    i.find(".price").html(n.price);
                    i.find(".product-item").attr("id", "product-" + n.id);
                    i.find(".variants").attr("id", "product-actions-" + n.id);
                    i.find(".variants select").attr("id", "product-select-" + n.id);
                    if (n.compare_at_price != n.price) {
                        i.find(".compare-price").html(n.compare_at_price).show();
        ...
        doUpdateDropdownCart: function (n) {
            ...
            $("#cart-count").text(n.item_count + " sản phẩm");
        },
        ...
        doUpdateDropdownCart: function (n) {
            var cart = '<li class="item" id="cart-item-{ID}"><a href="{URL}" title="{TITLE}" class="product-image"><img src="{IMAGE}" alt="{TITLE}"></a><div class="product-inner"><a href="javascript:void(0)" title="Xóa sản phẩm {TITLE}" class="btn-remove">X</a><p class="product-name"><a href="{URL}">{TITLE}</a></p><div class="cart-collateral">{QUANTITY} x <span class="price">{PRICE}</span></div></div></li>';
            $("#cartToggle .price").html(n.total_price + " đ");
            $("#cart-count").text(n.item_count + " sản phẩm");

            $("#dropdown-cart .summary .price").html(n.total_price + " đ");
            ...
            if (n.item_count > 0) {
                ...
                for (var i = 0; i < n.items.length; i++) {
                    ...
                    s = s.replace(/\{IMAGE\}/g, n.items[i].image);
                    s = s.replace(/\{PRICE\}/g, n.items[i].price + " đ");
        ...
        loadQuickViewSlider: function (n, cart) {
            cart.find(".quickview-featured-image").append('<a href="' + n.url + '"><img src="' + n.featured_image + '" title="' + n.title + '"/><div style="height: 100%; width: 100%; top:0; left:0 z-index: 2000; position: absolute; display: none; background: url(' + window.loading_url + ') 50% 50% no-repeat;"></div></a>');
            console.log(n.images);
            if (n.images.length > 0) {
                var o = cart.find(".more-view ul");
                for (i in n.images) {
                    var u = n.images[i].grand;
                    var a = n.images[i].compact;
